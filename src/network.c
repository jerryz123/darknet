...
void forward_network_half(network *netp)
{
    network net = *netp;
    int i;
    layer l;
    float buf[100];
    for(i = 0; i < net.n; ++i){
      printf("beginning layer %d\n", i);
      long start = rdcycle();
        net.index = i;
        l = net.layers[i];
        if(l.delta){
            fill_cpu_h(l.outputs * l.batch, 0, l.delta_h, 1);
        }

        l.forward_h(l, net);
        
        //cvt_single_prec (l.output_h, buf, 1);

        
        net.input_h = l.output_h;
        printf("layer %d %u\n", i, rdcycle() - start);
    }
...
int16_t* network_predict_half(network* net, int16_t* input_h)
{
  network orig = *net;
  net->input_h = input_h;
  net->truth = 0;
  net->train = 0;
  net->delta = 0;
  forward_network_half(net);
  int16_t* out = net->output_h;
  //*net = orig;
  return out;
}
